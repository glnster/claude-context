---
description: Research highest priority plane ticket needing investigation
---

## PART I - IF A plane TICKET IS MENTIONED
...
0a. fetch the top 10 priority items from plane in status "research needed" using the MCP tools, noting all items in the `links` section
...
2b. if the plane comments suggest web research is needed, use WebSearch to research external solutions, APIs, or best practices
...
think deeply, use TodoWrite to track your tasks. When fetching from plane, get the top 10 items by priority but only work on ONE item - specifically the highest priority issue.
